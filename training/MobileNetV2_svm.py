#!/usr/bin/env python3
"""
MobileNetV2 + SVM Training Script - GPU Optimized
...
def train_mobilenetv2_svm():
    """Train MobileNetV2 + SVM model with GPU optimization"""
    
    print("\n" + "="*80)
    print("🚀 TRAINING: MobileNetV2 + SVM (GPU Optimized)")
    print("="*80)
    
    # Check if already trained
    model_path = MODEL_DIR / "mobilenetv2_feature_extractor.h5"
    svm_path = MODEL_DIR / "mobilenetv2_svm_classifier.pkl"
    ...
    if model_path.exists() and svm_path.exists():
        ...
        latest_report_path = get_latest_report('MobileNetV2_SVM')
    ...
    print("Step 2: Building MobileNetV2 feature extractor...")
    ...
    with open(MODEL_DIR / "mobilenetv2_label_info.pkl", 'wb') as f:
    ...
    cm_path = REPORT_DIR / f"mobilenetv2_svm_confusion_matrix_{time.strftime('%Y%m%d_%H%M%S')}.png"
    ...
    report = generate_report(
        ...
        hyperparameters={
            ...
            'svm_gamma': SVM_GAMMA,
            'input_shape': '(224, 224, 3)',
            'feature_extractor': 'MobileNetV2 (ImageNet)',
            'pooling': 'avg',
            'batch_size': BATCH_SIZE,
            'mixed_precision': 'FP16 (enabled)'
        },
        notes='Feature extraction using pretrained MobileNetV2, classification with SVM'
    )
    
    report_path = save_report(report, 'MobileNetV2_SVM', confusion_matrix_path=cm_path)
