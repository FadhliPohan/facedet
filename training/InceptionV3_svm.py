#!/usr/bin/env python3
"""
InceptionV3 + SVM Training Script - GPU Optimized
...
def train_inceptionv3_svm():
    """Train InceptionV3 + SVM model with GPU optimization"""
    
    print("\n" + "="*80)
    print("🚀 TRAINING: InceptionV3 + SVM (GPU Optimized)")
    print("="*80)
    
    # Check if already trained
    model_path = MODEL_DIR / "inceptionv3_feature_extractor.h5"
    svm_path = MODEL_DIR / "inceptionv3_svm_classifier.pkl"
    ...
    if model_path.exists() and svm_path.exists():
        ...
        latest_report_path = get_latest_report('InceptionV3_SVM')
    ...
    # 1. Create datasets - NOTE: InceptionV3 uses 299x299
    print("Step 1: Loading pre-split datasets (299x299 for InceptionV3)...")
    ...
    print("Step 2: Building InceptionV3 feature extractor...")
    ...
    with open(MODEL_DIR / "inceptionv3_label_info.pkl", 'wb') as f:
    ...
    cm_path = REPORT_DIR / f"inceptionv3_svm_confusion_matrix_{time.strftime('%Y%m%d_%H%M%S')}.png"
    ...
    report = generate_report(
        model_name='InceptionV3_SVM',
        model_type='InceptionV3 + SVM',
        metrics=metrics,
        ...
        hyperparameters={
            ...
            'svm_gamma': SVM_GAMMA,
            'input_shape': '(299, 299, 3)',
            'feature_extractor': 'InceptionV3 (ImageNet)',
            'pooling': 'avg',
            'batch_size': BATCH_SIZE,
            'mixed_precision': 'FP16 (enabled)'
        },
        notes='Feature extraction using pretrained InceptionV3, classification with SVM'
    )
    
    report_path = save_report(report, 'InceptionV3_SVM', confusion_matrix_path=cm_path)
