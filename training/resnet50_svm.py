#!/usr/bin/env python3
"""
ResNet50 + SVM Training Script - GPU Optimized
...
def train_resnet50_svm():
    """Train ResNet50 + SVM model with GPU optimization"""
    
    print("\n" + "="*80)
    print("🚀 TRAINING: ResNet50 + SVM (GPU Optimized)")
    print("="*80)
    
    # Check if already trained
    model_path = MODEL_DIR / "resnet50_feature_extractor.h5"
    svm_path = MODEL_DIR / "resnet50_svm_classifier.pkl"
    ...
    if model_path.exists() and svm_path.exists():
        ...
        from utils.report_generator import get_latest_report, load_report
        latest_report_path = get_latest_report('ResNet50_SVM')
    ...
    # 1. Create datasets using pre-split data (NO SPLIT IN MEMORY!)
    ...
    train_ds, val_ds, test_ds, class_names, dataset_info = create_datasets_for_training(
        ...
        augment_train=False  # No augmentation needed for feature extraction
    )
    
    # 2. Build feature extractor
    print("Step 2: Building ResNet50 feature extractor...")
    ...
    # 4. Extract features from train and val sets (BATCH PROCESSING!)
    print("Step 3: Extracting features (batch processing for memory efficiency)...")
    ...
    # 7. Save label encoder (class names)
    label_info = {
        'class_names': class_names,
        'num_classes': len(class_names)
    }
    encoder_path = MODEL_DIR / "resnet50_label_info.pkl"
    ...
    cm_path = REPORT_DIR / f"resnet50_svm_confusion_matrix_{time.strftime('%Y%m%d_%H%M%S')}.png"
    ...
    report = generate_report(
        ...
        hyperparameters={
            ...
            'svm_gamma': SVM_GAMMA,
            'input_shape': '(224, 224, 3)',
            'feature_extractor': 'ResNet50 (ImageNet)',
            'pooling': 'avg',
            'batch_size': BATCH_SIZE,
            'mixed_precision': 'FP16 (enabled)',
            'gpu_name': gpu_info['devices'][0].get('gpu_name', 'Unknown') if gpu_info['available'] else 'N/A'
        },
        notes=f'Feature extraction using pretrained ResNet50, classification with SVM. '
              f'Used pre-split data - Train: {dataset_info["train_count"]}, '
              f'Val: {dataset_info["val_count"]}, Test: {dataset_info["test_count"]}'
    )
    
    report_path = save_report(report, 'ResNet50_SVM', confusion_matrix_path=cm_path)
    ...
    print(f"\n📊 Dataset Split Used:")
    print(f"   Train: {dataset_info['train_count']:,} images")
    print(f"   Val:   {dataset_info['val_count']:,} images")
    print(f"   Test:  {dataset_info['test_count']:,} images")
    
    print(f"\n💾 Saved Files:")
    print(f"   Feature Extractor: {model_path}")
    print(f"   SVM Classifier: {svm_path}")
    print(f"   Label Info: {encoder_path}")
